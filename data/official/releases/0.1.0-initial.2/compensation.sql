...
BEGIN
  IF EXISTS (SELECT 1 FROM app_private.interaction_events)
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'DATA_SEED_COMPENSATION_OPERATIONAL_ROWS_PRESENT';
  END IF;
END;
...
BEGIN
  ...
  IF v_mismatch THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'DATA_SEED_COMPENSATION_PROJECTION_MISMATCH';
  END IF;
END;
$data_seed_projection_guard$;

DELETE FROM app_private.office_service_mappings AS mapping
USING app_private.offices AS office
WHERE mapping.office_id = office.id
  AND (office.public_id, mapping.intent::text) IN (
    VALUES
    ('OFFICE-AREUM'::text, 'BULKY_WASTE'::text),
    ('OFFICE-AREUM'::text, 'CERTIFICATE_ISSUANCE'::text),
    ('OFFICE-AREUM'::text, 'MOVE_IN_RESIDENT_REGISTRATION'::text),
    ('OFFICE-DODAM'::text, 'CERTIFICATE_ISSUANCE'::text),
    ('OFFICE-DODAM'::text, 'LOCAL_TAX_GENERAL'::text),
    ('OFFICE-DODAM'::text, 'MOVE_IN_RESIDENT_REGISTRATION'::text),
    ('OFFICE-JOCHIWON'::text, 'BULKY_WASTE'::text),
    ('OFFICE-JOCHIWON'::text, 'CERTIFICATE_ISSUANCE'::text),
    ('OFFICE-JOCHIWON'::text, 'LOCAL_TAX_GENERAL'::text),
    ('OFFICE-JOCHIWON'::text, 'MOVE_IN_RESIDENT_REGISTRATION'::text)
  );
...
BEGIN
  IF EXISTS (SELECT 1 FROM app_private.kb_documents)
     ...
     OR EXISTS (SELECT 1 FROM app_private.kb_candidates)
     OR EXISTS (SELECT 1 FROM app_private.audit_logs) THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'DATA_SEED_COMPENSATION_ABSENCE_FAILED';
  END IF;
END;
$data_seed_empty_guard$;
