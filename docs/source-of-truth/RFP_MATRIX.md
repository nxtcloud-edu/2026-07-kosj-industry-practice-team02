...
| --- | --- | --- | --- | --- | --- |
| SFR-001 | 자연어 질의응답 | 4개 우선 분야의 일상어 질문을 공식 ACTIVE KB에 연결하고 구조화 답변과 출처 제공 | P0 실제 구현 | 정상 질문 10개에서 분류·근거·구조화 필드·출처 확인 | Upstage `solar-pro3`는 local/private canonical 합성 평가 전용; 실제 시민 경로는 template 안전 경로 |
...
| SER-001 | 개인정보 최소수집 | 외부 LLM 호출 전 마스킹, 원문 DB 미저장, 앱 DB IP·기기ID 미수집 | P0 실제 구현 | 데이터 흐름·스키마·로그·provider allowlist와 local DB exact single-loopback gate 검사 | Upstage는 server-owned `T-01`~`T-10` 합성 평가만; 공개/실제 시민 입력 금지. DB tooling은 D-031/D-032의 pinned patched CLI와 short checkout path-budget을 통과해야 함 |
