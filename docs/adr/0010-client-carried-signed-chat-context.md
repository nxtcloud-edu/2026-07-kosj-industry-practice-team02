...
계약·보안 테스트는 `session_id` 거부, token 누락/null 첫 요청, 정확한 900초 TTL, claim allowlist, tamper/expiry silent reset, 현재 request region 우선, FALLBACK null, token/secret의 DB·로그 0건, decoded payload의 질문·답변·URL·PII 0건, 새로고침 후 소멸을 검증한다. 표준 라이브러리만 사용하므로 이 결정 자체는 새 production dependency를 추가하지 않는다.
