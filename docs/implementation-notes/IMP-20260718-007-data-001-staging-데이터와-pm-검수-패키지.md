...
| `scripts/tests/test_data_staging_validation.py` / `scripts/verify.ps1` | 62 focused tests와 mandatory root `VALIDATE-DATA-001` integration | regression/runtime isolation, external-CWD canonical path/reparse/privacy/source-matrix hardening |
...
| `python -B -m unittest scripts.tests.test_data_staging_validation -q` | PASS | 62 tests, 15.516s | focused DATA-001 suite after Remediation 2 rereview fixes |
...
| tracked runtime/operations staging-reference scan | PASS | issues 0 | runtime trees plus ops/config/deployment/infra PowerShell·JSON·TOML·YAML; actual temporary `git ls-files`, case/comment/concat/split bypass tests included |
...
- Security: secret/mock/unsafe source/self-approval/hash mismatch/runtime staging reference fail closed; errors
  ...
  Relative production CLI paths are rooted at the repository and replaced with resolved canonical absolute
  paths before any accepted read/write, including execution from an external current working directory.
