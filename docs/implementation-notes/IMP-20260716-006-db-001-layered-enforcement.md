...
- Task ID: DB-001
- Type: implementation
- Status: Blocked — Task 10 Q-SEC-006/A-024 explicit HostIP tooling supply-chain boundary; actual DB review, version promotion, and completion commit pending
...
- bare `supabase start`가 만든 Kong은 데이터 volume 삭제 없이 제거했다. Task 10 port finding 뒤
  ...
  직접 결정할 항목은 Q-SEC-006이다. Q-SEC-004=A/D-029와 Q-SEC-005=A/D-030의 두 보정은 actual IPv6 wildcard를 남겼다.
...
- Q-SEC-002/Q-WF-001/Q-DB-003은 A로 해결됐다. 현재 인간 A/Blocker는
  A-024/Q-SEC-006 1개이고 A-021은 별도 B/High Open/Deferred다.
...
동기화했다. Q-SEC-004=A/D-029와 Q-SEC-005=A/D-030의 보정은 모두 IPv6 wildcard를 남겼고, Q-SEC-006/A-024 해결과 safe runtime/full gate 전 DB-001은 Blocked다.
...
현재는 과거 DB 증거와 후보 문서만 있으며 Q-SEC-006/A-024 때문에 완료되지 않았다.
...
fail-closed runner로 재현했다. Q-SEC-006/A-024 해결 전 fresh DB review/commit을 완료로 기록하지 않는다.
...
actual loopback/full DB gate와 independent completion review는 Q-SEC-006/A-024 해결 뒤 pending이다.

### 인간이 반드시 알아야 하는 내용

- `0.3.0-local`은 exact loopback/full gate 뒤에만 사용할 후보이며 현재 manifest는
  `0.2.0-draft`다. Q-SEC-006/A-024는 인간 A/Blocker다.
...
- [ ] Q-SEC-006 해결 뒤 fresh DB/root/static verification
