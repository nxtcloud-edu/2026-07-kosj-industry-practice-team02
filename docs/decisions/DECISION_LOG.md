...
| D-018 | 2026-07-14 | DB 실행 권위는 Supabase CLI 버전 SQL 마이그레이션으로 관리 | Confirmed; six local forward/compensation files implemented, but DB-001 completion/version promotion is blocked by A-023/Q-SEC-005 after the Q-SEC-004 remediation proved insufficient | Q-DB-001, ADR-0008, DB-001 candidate report/handoff | 사용자 |
...
| D-025 | 2026-07-16 | 승인·보관·ACTIVE 공식 검색·권한 규칙은 DB function/trigger/RLS/GRANT와 백엔드 검증에서 이중 강제 | Confirmed; implementation evidence exists but final local baseline/version is blocked by A-023/Q-SEC-005; A-021 separately blocks public release | Q-DB-002, ADR-0011, DB-001 plan/candidate report/handoff | 사용자 |
...
Q-SEC-004/A-022는 D-029로 결정됐지만 실제 보정이 불충분했다. A-023/Q-SEC-005가 현재 local 완료 blocker이며 답변 전 결정 행을 만들지 않는다.
