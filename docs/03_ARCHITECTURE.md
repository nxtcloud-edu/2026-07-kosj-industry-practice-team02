...
- `database/schema-v1.draft.sql`은 `0.3.0-local` 후보의 읽기 전용 논리 투영이며 권한·함수·trigger
  실행 근거가 아니다.
- manifest는 Q-SEC-006/A-024 explicit HostIP tooling blocker로 `0.2.0-draft`를 유지한다. exact loopback과
  full gate 전에는 후보 shape를 완료 기준선으로 부르지 않는다.
- A-021/Q-SEC-003 기본값 B에 따라 A-024 해소 뒤 승격 가능한 기준선도 local/private에
