...
- 계획 시점 official release/DB seed: 0건, `official_data=0.0.0-not-populated`, `/ready=503`
...
- official release directory는 없고 `supabase/seed.sql`은 data-free 설명문이다.
- DB schema는 migration 6개, table 8개, disposable local `0.3.0-local`; public release는 Q-SEC-003/A-021로 별도 차단된다.
- current root verify는 DATA-001 validator를 실행하지만 official release/dispatcher drift는 아직 검사하지 않는다.
- `scripts/run_database_sql.py`는 의도적으로 `database/` 아래 SQL만 허용하므로 release SQL 실행에 재사용하지 않는다.
- `database/README.md`의 Q-SEC-006 미해결 문구는 현재 D-031/DB-001 완료 상태보다 오래됐으며 closeout에서 정정한다.
- 공식 data/API/DB/readiness를 변경할 A blocker는 실행계획 승인 하나다.
...
- [ ] **Step 1: RED input/schema tests를 작성한다**
...
- [ ] **Step 4: strict loader와 trust validator 최소 구현을 작성한다**
...
- [ ] **Step 5: focused test와 기존 DATA-001 회귀를 실행한다**
...
- [ ] **Step 1: RED projection/hash tests를 작성한다**
...
- [ ] **Step 3: canonical projection과 normalization을 구현한다**
...
- [ ] **Step 4: safe SQL serializer를 구현한다**
...
- [ ] **Step 5: compensation generator를 구현한다**
...
- [ ] **Step 6: deterministic/fuzz-like escaping 회귀를 실행한다**
...
- [ ] **Step 1: RED publication tests를 작성한다**
...
- [ ] **Step 3: exact CLI/parser/path boundary를 구현한다**
...
- [ ] **Step 4: same-parent prepare를 구현한다**
...
- [ ] **Step 5: atomic dispatcher activation을 구현한다**
...
- [ ] **Step 6: path/reparse/partial/drift tests를 실행한다**
...
- [ ] **Step 1: RED DSN/session/unit tests를 작성한다**
...
- [ ] **Step 2: RED PowerShell runner tests를 작성한다**
...
- [ ] **Step 3: Python DB verifier를 구현한다**
...
- [ ] **Step 4: concurrency probe를 구현한다**
...
- [ ] **Step 5: PowerShell orchestration을 구현한다**
...
- [ ] **Step 6: unit/static runner tests를 실행한다**
...
- [ ] **Step 1: canonical input을 다시 검증한다**
...
- [ ] **Step 2: 고정된 governance timestamp로 prepare한다**
...
- [ ] **Step 4: dispatcher를 활성화하고 byte equality를 검증한다**
...
- [ ] **Step 3: exclusions와 final DB state를 확인한다**
...
- [ ] **Step 1: RED root-runner tests를 작성한다**
...
- [ ] **Step 2: root verify stages를 최소 구현한다**
...
- [ ] **Step 3: lineage와 stale docs를 갱신한다**
...
- [ ] **Step 4: version을 actual result 뒤에만 갱신한다**

`official_data: 0.1.0-initial.1`, `test_suite: 0.9.0-data-seed-release`, `documentation: 2.8.0`으로 갱신한다. application 0.2.0, web 0.2.0-static-chat-shell, API 2.0.1-draft, shared 0.2.1, DB 0.3.0-local, mock/prompt는 유지한다.

- [ ] **Step 5: 전체 verification을 실행한다**
...
Expected: root/Web/API/contracts/DATA-001/DATA-SEED/secret/package/diff 모두 PASS; `/ready=503` 회귀 PASS.

- [ ] **Step 6: 구현 note를 완성하고 commit한다**
...
| Official data | 0.0.0-not-populated | 0.1.0-initial.1 | real DB full cycle PASS 뒤만 |
...
- 현재 필요한 승인: 이 실행계획 전체와 local disposable DB의 reset/seed/compensation/replay 실행.
- 승인 표현: `계획 승인, 구현 시작` 또는 동등한 명시적 표현.
- written specification 승인 시각 `2026-07-19T09:20:31+09:00`을 initial release의 explicit governance `released_at`으로 사용하는 결정. 다른 시각을 원하면 구현 전에 지정한다.
- 이후 별도 승인: successor release, WASTE-03 activation, non-empty/remote data transition, migration/API/public deployment, 새 dependency.
...
- 실제 결과: 계획 승인과 구현 뒤 채운다.
- 계획과 달라진 점: 변경 발생 시 이유·승인·테스트를 기록한다.
- 다음 단계: DATA-SEED 완료 뒤 READY-001 → AI-001 순서로 별도 명세/계획을 진행한다.
