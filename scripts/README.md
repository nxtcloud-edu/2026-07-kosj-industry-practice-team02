...
`prepare`는 세 content JSON의 count와 SHA-256, AI 권고를 묶은 `PENDING_PM_REVIEW` manifest를 최초 한 번만 원자적으로 쓴다. 동일 bytes의 미검수 PENDING 재실행만 무쓰기 성공하며, 다른 PENDING 또는 PM 결정·comment·reviewer metadata가 있는 manifest는 덮어쓰지 않는다. `migrate-pending`은 과거 AI 권고가 `decision`에 있던 정확한 canonical legacy PENDING 전체 객체만 새 `recommended_decision`/null PM 결정 shape로 한 번 전환하며, metadata·artifact·decision이 하나라도 다르면 쓰기 전에 거부한다. `validate`는 PENDING·APPROVED·REJECTED 상태를 상태별로 검사하고 staging을 변경하지 않는다. 유효한 PENDING은 AI의 `recommendation_projection`만, 유효한 APPROVED는 PM의 `approval_projection`만 report에 제공하며, REJECTED 또는 validation issue가 있는 제출물의 두 projection은 `null`이다. 원문·출처 값·PII/비밀값을 report나 CLI 출력에 복사하지 않는다. 성공 출력은 `[PASS] step=VALIDATE-DATA-001`이고, validation failure는 stable issue-code count만 출력하며 1로 종료한다. 사용법 오류는 2로 종료한다.
